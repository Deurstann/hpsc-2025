...
int main(int argc, char** argv) {
  ...
  for(int irank=0; irank<size; irank++) {
    ...
    Body bufferbody[N/size];
    MPI_Win win;
    MPI_Win_create(bufferbody, (N/size)*sizeof(Body), sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
    MPI_Win_fence(0, win);
    MPI_Put(jbody, (N/size)*sizeof(Body), MPI_BODY, send_to, 0, N/size, MPI_BODY, win);
    MPI_Win_fence(0, win);
    for(int i=0;i<N/size;i++) jbody[i]=bufferbody[i];
    for(int i=0; i<N/size; i++) {
